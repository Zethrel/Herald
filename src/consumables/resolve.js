 ...
 */
export function resolveSpecConsumables({ spec, dataset, overrides = {} }) {
  ...
  for (const slot of SLOTS) {
    ...
    slots[slot] = hit
      ? { via: hit[0], item: resolveItem(dataset, hit[1]) }
      : { via: null, item: null };
  }
...
export function resolveAll({ specs, dataset, overrides = {} }) {
  return specs.map((spec) => resolveSpecConsumables({ spec, dataset, overrides }));
}
 ...
 */
export function gaps({ specs, dataset, overrides = {} }) {
  const missing = [];
